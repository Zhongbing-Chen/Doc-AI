...
from model.table_transformer.pipeline import postprocess

sys.path.append("../detr")
from model.table_transformer.models import build_model
...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    if objects:
        ...
        tables = [{'bbox': [min(det['bbox'][0] for det in objects), min(det['bbox'][1] for det in objects),
                            max(det['bbox'][2] for det in objects), max(det['bbox'][3] for det in objects)],
                   'score': 1.0, 'label': 'table'}]
    table_structures = []
...
def infer_by_image(img, pipe):
    ...
    extracted_table = pipe.recognize(img, tokens, out_objects=True, out_cells=True,
                                     out_html=False, out_csv=False)
    print("Table(s) recognized.")
    # parser.output_objects(img_file, extracted_table['objects'])
...
def inference_for_table_recognition(image_dir):
    return infer_by_file('recognize', 'cpu', 'cpu',
                         structure_config_path='/home/zhongbing/Projects/MLE/table-transformer/detr/config/structure_config.json',
...
if __name__ == "__main__":
    ...
    result = infer_by_image(img, pipe)
    print(result)
    end = time.time()
    print(end - start)
